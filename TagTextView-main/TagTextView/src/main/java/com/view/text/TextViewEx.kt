...
import com.view.text.view.TingTextView
...
@JvmOverloads
fun TingTextView.addTagNew(config: TagConfig, onClickListener: (() -> Unit)? = null): TingTextView = apply {
    verifyText(this)
    val builder = createSpannableStringBuilder(this, config.position)
    val newPosition = insertPlaceholder(builder, config.position)
    val imageSpan = createSpan(this, config, newPosition)
    if(imageSpan is CenterImageSpan){
        imageSpan.setPosition(newPosition)
    }
    builder.setSpan(
        imageSpan,
        newPosition,
 ...
 * @param [config] 配置参数
 */
private fun createSpan(textView: TextView, config: TagConfig, position: Int = 0): ReplacementSpan {
